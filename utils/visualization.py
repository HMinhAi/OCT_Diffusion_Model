...
def overlay_heatmap(
    ...
    heat = normalize_map(anomaly_map)
...
def save_anomaly_panel(
    ...
    axes[1].imshow(overlay_heatmap(image_np, diff_np))
    axes[1].set_title("Diffusion Map")
    axes[1].axis("off")

    axes[2].imshow(overlay_heatmap(image_np, feat_np))
    axes[2].set_title("Feature Map")
    axes[2].axis("off")

    col_offset = 3
    if noise_map is not None:
        axes[3].imshow(overlay_heatmap(image_np, noise_np))
        axes[3].set_title("Simplex Noise Map")
        axes[3].axis("off")
        col_offset = 4

    axes[col_offset].imshow(overlay_heatmap(image_np, fused_np))
