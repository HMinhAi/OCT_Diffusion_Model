...
def median_filter_2d(x: torch.Tensor, kernel_size: int = 3) -> torch.Tensor:
    """Bộ lọc trung vị sử dụng PyTorch thuần."""
    padding = kernel_size // 2
    # Trích xuất các vùng lân cận thành các cột
    patches = F.unfold(x, kernel_size=kernel_size, padding=padding)
    # patches có shape: (B, C * kernel_size^2, L)
    # Lấy trung vị theo chiều của các pixel trong vùng kernel
    median = patches.median(dim=1, keepdim=True).values
    # Đưa về lại hình dạng ban đầu
    return median.view_as(x)
...
class DiffusionInversionDetector:
    ...
    def invert(self, x: torch.Tensor) -> InversionOutput:
        ...
        # Sửa trong hàm invert của class DiffusionInversionDetector
        if self.anomaly_distance == "l2":
            diff = ((x - x_recon) ** 2).mean(dim=1, keepdim=True)
        else:
            diff = (x - x_recon).abs().mean(dim=1, keepdim=True)
        diffusion_map = median_filter_2d(diff, kernel_size=5)
