...
class DDPM(nn.Module):
    ...
    def q_sample(self, x0: torch.Tensor, t: torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        # Thêm nhiễu Speckle giả lập vào x0 trước khi khuếch tán
        # Speckle noise: x = x + x * n (n là nhiễu Gaussian)
        if self.training:
            speckle = torch.randn_like(x0) * 0.05 # Cường độ nhiễu 5%
            x0 = x0 + x0 * speckle
            x0 = x0.clamp(-1.0, 1.0)
    ...
    # Tìm hàm training_loss trong class DDPM
    def training_loss(self, x0: torch.Tensor) -> torch.Tensor:
        ...
        # 1. Tính MSE/SmoothL1 loss cơ bản
        loss_dist = F.smooth_l1_loss(noise_pred, noise, beta=0.1)

        # 2. Tính Structural Loss bằng Laplacian
        # Định nghĩa kernel cho 3 kênh (RGB/OCT channels)
        channels = x0.shape[1] # Thường là 3
        laplacian_data = torch.tensor([[-1,-1,-1], [-1,8,-1], [-1,-1,-1]], 
                                    dtype=torch.float32).to(x0.device)
        
        # Mở rộng kernel để khớp với số kênh đầu vào
        # Shape: [channels, 1, 3, 3] để dùng với groups=channels
        laplacian_kernel = laplacian_data.view(1, 1, 3, 3).repeat(channels, 1, 1, 1)

        # Tính toán Laplacian cho cả 3 kênh
        lap_pred = F.conv2d(noise_pred, laplacian_kernel, padding=1, groups=channels)
        lap_target = F.conv2d(noise, laplacian_kernel, padding=1, groups=channels)
        
        loss_struct = F.mse_loss(lap_pred, lap_target)
        
        return loss_dist + 0.1 * loss_struct
